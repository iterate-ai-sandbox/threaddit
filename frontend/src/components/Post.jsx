        ...
        <div className="flex flex-col flex-1 p-3 space-y-5 w-full md:space-y-0 md:space-x-4 md:flex-row">
          <div className="flex flex-col space-y-1 w-full md:justify-between">
            {isExpanded ? (
              ...
              <Link to={`/post/${post?.post_info.id}`} onClick={() => mixpanel.track('post_selected', { post_name: post.post_info.title, post_comments: post.post_info.comments_count, post_category: post.thread_info.thread_name })} className="flex flex-col space-y-2 w-full h-full">
