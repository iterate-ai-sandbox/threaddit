        ...
        {post.post_info.media && (
          ...
          >
            {!isImage(post.post_info.media) ? (
              ...
              <img onClick={() => { mp.track("eight_ten_pm", { time: new Date().toISOString() }); onMediaClick("image"); }} loading="lazy" width="auto" height="100%" src={post.post_info.media} alt="" className={`object-cover w-full h-full rounded-md duration-500 md:cursor-pointer ${ !isExpanded && "hover:scale-105" }`} />
            )}
          </div>
        )}
