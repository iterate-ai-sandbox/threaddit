        ...
        {post.post_info.media && (
          ...
          >
            {!isImage(post.post_info.media) ? (
              ...
              <img onClick={() => { mixpanel.track('tq', { tq: 'tq' }); mp.track("Post Media Clicked", { time: new Date() }); onMediaClick("image"); }} loading="lazy" width="auto" height="100%" src={post.post_info.media} alt="" className={`object-cover w-full h-full rounded-md duration-500 md:cursor-pointer ${ !isExpanded && "hover:scale-105" }`} />
            )}
          </div>
        )}
...
<a class="text-xs font-medium hover:underline text-theme-orange" href="/Fitness" onClick={() => mixpanel.track('rq', { rq: 'rq' })}>
